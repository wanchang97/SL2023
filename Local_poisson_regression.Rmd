...
In this case, $(X,Y) \sim \text{Poisson} (\lambda(x)),Y = 0,1,2,\cdots;$ 
<!-- The regression function is chosen as $\lambda(x) \in (0,\infty) $. -->
The regression function is chosen as:
$$\lambda(x) \in (0,\infty)$$
The link function is chosen as $$\log(\cdot)$$ and 
the smooth function is $$\log(\lambda(x)) \in \mathbb{R}$$ free of constraints.
We will then use local Poisson regression and will focus the assignment in the choice of the bandwidth of the kernel such that the expected log-likelihood of an independent observation is maximized: 
...
ggplot(data = data.frame(h.CV.loglik.poisson), aes(x=h, y=cv.h)) +
  geom_point(size=3) + 
  geom_line(linewidth=1, lty=1) +
  geom_vline(xintercept = h.CV.loglik.poisson$h.cv, lty=2, linewidth=1.2, col=2) + 
...
```
```{r,include=FALSE}
aux <- sm.poisson(Life.expec,le.fm.r,h=h.CV.loglik.poisson$h.cv)
title(main=paste("h.cv.loglik.poisson =",round(h.CV.loglik.poisson$h.cv,2),sep=""))
