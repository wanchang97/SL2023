...
author: "E. Vegas, F. Reverter and A. Sanchez"
format:
  revealjs: 
    incremental: false  
    transition: slide
...
- These are used to measure the extent of purity for a region containing data points from possibly different classes.
...
## Entropy based goodness of split

- Goodness of split measure based on Entropy impurity function.
...
- For a feature $F$ and a set of observations $X$, information gain is defined as:

$$
\begin{aligned}
& I G(X, F)=\text { (original entropy) }-(\text { entropy after the split) } \\
